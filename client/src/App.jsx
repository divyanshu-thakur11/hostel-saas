...
function AppShell() {
  ...
  useEffect(() => {
    ...
    // Check if new owner needs onboarding (no hostels yet)
    if (user.role === 'owner') {
      hostelAPI.getAll().then(res => {
        if (!res.data || res.data.length === 0) setShowOnboarding(true);
      }).catch(() => {});
    }
  ...
  if (user.role === 'superadmin') return <SuperAdminDashboard user={user} onLogout={doLogout} />;
  if (showOnboarding) return <Onboarding user={user} onComplete={() => { setShowOnboarding(false); loadHostel(); }} />;
...
export default function App() {
  return (
    <BrowserRouter>
      <ToastProvider>
        <Routes>
          {/* Member portal — completely isolated, own auth, own cookie */}
          <Route path="/member-portal/*" element={<MemberPortal />} />
          {/* Staff + admin app — members who accidentally land here see nothing useful */}
          <Route path="/*" element={<HostelProvider><AppShell /></HostelProvider>} />
        </Routes>
      </ToastProvider>
    </BrowserRouter>
  );
